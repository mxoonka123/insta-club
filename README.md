...
| Команда | Зачем |
|---|---|
| `/admin` | статистика, кнопки «Встречи» и «Создать встречу» |
| `/applications` | заявки на вступление |
| `/find Мария` или `/find 123456789` | найти человека |
| `/approve ID` | открыть доступ |
| `/reject ID` | отклонить заявку |
| `/kick ID` | закрыть доступ уже вступившему |
| `/renew ID` | продлить подписку на 30 дней |
| `/meetings` | все встречи: ближайшие, прошедшие, RSVP |
| `/meeting` | создать встречу: офлайн/онлайн → город → дата → время → тема → лимит |
...
Список всех встреч — `/meetings` или `/admin` → «Встречи»: ближайшие, прошедшие, сколько записалось. Там же можно снять ближайшую с публикации (участники её больше не видят).
...
bot/
  ...
  handlers/
    ...
    admin.py         заявки, кик, статистика
    events.py        встречи для участников, /meetings и /meeting
